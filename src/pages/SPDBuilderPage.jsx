import { usePlanYear, planYearLong, planYearLabel, acaPlanYearLong, acaPlanYearLabel } from '@/lib/PlanYearContext'
...
export default function SPDBuilderPage() {
  ...
  useEffect(() => {
    ...
    if (co?.group_type === 'aca_small_group') {
      // Default to the company's real plan year anchor — plan_effective_date
      // is the authoritative, admin-set field (same one the portal's OE
      // lock logic uses); renewal_date is the fallback if that's not set.
      setAcaPlanStart(co.plan_effective_date || co.renewal_date || '')
  ...
  // For ACA: compute plan year from the company's renewal date (defaulted
  // above), using the same shared helper the rest of the app uses.
  const acaPlanEnd       = acaPlanStart ? acaPlanYearLong(acaPlanStart).split(' – ')[1] : ''
  const acaPlanStartLong = acaPlanStart ? acaPlanYearLong(acaPlanStart).split(' – ')[0] : ''
  const acaPlanYearLongVal  = acaPlanStart ? acaPlanYearLong(acaPlanStart) : 'Contact KIAA for plan year dates'
  const acaPlanYearShort = acaPlanStart ? acaPlanYearLabel(acaPlanStart) : ''
  ...
  function handleDownloadHtml() {
    ...
    const html = generateSPDHtml({ company, planList, sections, generatedDate, isAca, planYear: isAca ? acaPlanYearLongVal : oePlanLong, planYearShort: isAca ? acaPlanYearShort : oePlanShort })
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div>
            ...
            <input className="input opacity-70 cursor-default"
              value={isAca ? acaPlanYearLongVal : oePlanLong} readOnly />
            {isAca && (
              <div className="mt-2">
                ...
                <p className="text-xs text-surface-400 mt-1">Defaults to this company's plan effective date (or renewal date). Override only if generating for a different effective date.</p>
              </div>
            )}
          </div>
        </div>
      ...
      {generated && company && (
        <div ref={previewRef} className="card">
          <div className="text-center pb-5 mb-5 border-b border-surface-100">
            ...
            <p className="text-surface-400 text-xs mt-0.5">Plan Year: {isAca ? acaPlanYearLongVal : oePlanLong}</p>
          </div>
          ...
          <SPDSection title="1.  Plan information">
            <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-sm">
              {[
                ...
                ['Plan year',          isAca ? acaPlanYearLongVal : oePlanLong],
            ...
            <SPDSection title="4.  Plans enrolled">
              {isAca ? (
                <>
                  ...
                  plan(s) for the {acaPlanYearLongVal} plan year:{' '}
                ...
                <>
                  {company.name} offers the following HMSA plan(s) for the {isAca ? acaPlanYearLongVal : oePlanLong} plan year:{' '}
